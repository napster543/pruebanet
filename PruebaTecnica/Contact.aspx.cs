        ...
        protected void btnGuardarData_Click(object sender, EventArgs e)
        {
            try /* Updation After Validations*/
            {
                using (NpgsqlConnection connection = new NpgsqlConnection())
                {
                    ...
                    string sqltext = "INSERT INTO novedadesnomina(idcontrato, periodolaborado, horaslaboradas, horaextradiurna, horaextranocturna, horaextradominical, horaextrafestiva, descuentos, usuariocreacion, fechacreacion) ";
                    sqltext = "VALUES(@idcontrato,@periodo,@horalaborada,@HEdiuerna,@HEnocturna,@HEdominical,@HEfestiva,@descuento,'prueba',now())";
                    cmd = new NpgsqlCommand(sqltext, connection);
                    
                    cmd.Parameters.AddWithValue("@idcontrato", txtcontrato.Text);
                    cmd.Parameters.AddWithValue("@periodo", txtperiodolaborado.Text);
                    cmd.Parameters.AddWithValue("@horalaborada", txthoralaborada.Text);
                    cmd.Parameters.AddWithValue("@HEdiuerna", txtdiurnas.Text);
                    cmd.Parameters.AddWithValue("@HEnocturna", txtNocturnas.Text);
                    cmd.Parameters.AddWithValue("@HEdominical", txtdominicales.Text);
                    cmd.Parameters.AddWithValue("@HEfestiva", txtfestivas.Text);
                    cmd.Parameters.AddWithValue("@descuento", txtdescuentos.Text);
